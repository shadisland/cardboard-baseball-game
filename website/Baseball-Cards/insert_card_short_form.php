...
$split_array = explode('_', $fileName);
$brand = $split_array[1];
$number = $split_array[2];

$player_name = str_replace("_", " ", 
substr( $fileName, 
strpos($fileName, $number) + strlen($number) + 1, 
strpos($fileName, "__" . $grade_company) 
- strpos($fileName, $number) -  1 - strlen($number)) );
//$player_name = str_replace("_", " ", substr( $img_url, strpos($img_url, $number) + strlen($number) + 1, strpos($img_url, "__SGC") - strpos($img_url, $number) -  1 - strlen($number)) );
//$player_name = str_replace("_", " ", substr( $img_url, strpos($img_url, $number) + strlen($number) + 1, strpos($img_url, '__SGC') - strpos($img_url, $number) - 4)); 


$grade = substr( $fileName, strpos($fileName, 'Grade-') + 6, strpos($fileName, '_Auth-') - strpos($fileName, 'Grade-') - 6 ); //substr( $split_array[4], strpos($split_array[4], 'Grade-') + 1 );
$grade_serial = substr( $fileName, strpos($fileName, 'Auth-') + 5, strpos($fileName, '_Front') -  strpos($fileName, 'Auth-') - 5);
$i_own_it = 1;
